...
format_instructions = PydanticOutputParser(
    pydantic_object=StorageDesign).get_format_instructions()
llm_with_tools = llm.bind_tools([calculator])
llm_structured = build_llm(num_ctx=12288, num_predict=4608, timeout=360)
llm_with_structure = llm_structured.with_structured_output(StorageDesign)


prompt = ChatPromptTemplate.from_messages([
    ("system", STORAGE_EXPERT_SYSTEM_PROMPT),
    ("human",
     "Clarified requirements:\n{clarified_requirements}\n\n"
     "Traffic estimates:\n{traffic_estimates}\n\n"
     "Design the storage layer, using the calculator tool to estimate storage "
     "growth. You have at most {max_tool_iterations} rounds of calculator "
...
async def storage_expert_agent(state: DesignState):
    ...
    messages = prompt.format_messages(
        ...
        or 'Not available — estimate storage growth conservatively from requirements alone.',
    )
    ...
    if hit_iteration_cap:
        ...
        messages.append(HumanMessage(
            ...
            "Produce the final storage design as structured output."
        ))
